...
class AddressBook(UserDict):
    ...
    def save_to_file(self, filename):
        with open(filename, 'w') as f:
            data = {'records': [record.__dict__ for record in self.data.values()]}
...
address_book = AddressBook()
address_book.add_record(Record("John Smith", phones=["+380971234567"]))
address_book.save_to_file("address_book.txt")
new_address_book = AddressBook()
new_address_book.load_from_file("address_book.txt")
